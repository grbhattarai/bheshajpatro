...
function formatMoonSignBlock(p) {
  ...
  if (line1) {
    names.push(line1);
    ends.push(p.moon_rashi1_hm || null); // when first rashi ends
  }
  if (line2) {
    names.push(line2);
    ends.push(null); // second rashi runs until next sunrise
  }

  return formatLimbBlockFromArrays(names, ends);
}

// Sunsign block (single line, no split across day)
...
const monthlyGridOption = document.getElementById("monthly-grid-option");
const monthlyTradOption = document.getElementById("monthly-trad-option");
...
function renderDaily(dateStr, place, p) {
  ...
  // Line 2: full Gregorian date
  if (gregDateLineEl) gregDateLineEl.textContent = fullDate;

  // Line 3: BS date from sun_day + month_name + bs_year
  ...
  // keep 24-hour format exactly as backend sends it
  const sunrise =
  ...
  const paksha = p.paksha || ""; // "shukla" / "krishna"
  ...
  // --- RIGHT HEADER HALF: month / paksha / main tithi name -----
  ...
  // Row 1: Sunrise / Sunset (24h as-is, no normalization)
  rows.push(fourColRow("Sunrise", sunrise, "Sunset", sunset));

  // Row 2: Tithi / Nakshatra (multi-line blocks)
  ...
  // Row 5: Dinamana (other columns reserved for future use)
  if (dinamana) {
...
// ---------- Sidebar: Home / Set Location / Set Method / Monthly Patro -----

if (homeBtn) {
  homeBtn.addEventListener("click", () => {
    // IMPORTANT: only reset the date, keep last known place & method
    saveState({
      date: todayISO(),
    });
    loadDaily();
...
if (monthlyPatroBtn) {
  monthlyPatroBtn.addEventListener("click", () => {
    ...
    // copy current month URLs into modal buttons
    if (monthGridLink && monthlyGridOption) {
      monthlyGridOption.href = monthGridLink.href;
    }
    if (printableMonthlyLink && monthlyTradOption) {
      monthlyTradOption.href = printableMonthlyLink.href;
    }
